    ...
    const table = (
        <View>
            {getDaysInWeek().map((day) => {
                let dailyTraining
                plan.plan.forEach((week) => week.days.forEach((training) => {if(moment(training.date).format("DD/MM/YY") === moment(day).format("DD/MM/YY")) dailyTraining = training.workout}))
            return (<Text key={day.toString()}>{moment(day).format("DD/MM/YY").toString()} - {dailyTraining}</Text>);
                })}
        </View>
    )
